--[[弹道系统
	...
	local info = {
		hAbility = hAbility,
		hCaster = hParent,
		sEffectName = "particles/units/heroes/hero_dragon_knight/dragon_knight_breathe_fire.vpcf",
		vSpawnOrigin = hParent:GetAbsOrigin(),
		vDirection = vDirection,
		iMoveSpeed = self.speed,
		flDistance = self.range,
	...
	local info = {
		...
		hTarget = hTarget,
		sEffectName = hAttacker:GetProjectileName(),
		hAbility = self,
...
PROJECTILE_TYPE_SURROUND = 3	---环绕弹道（围绕单位旋转）
...
-- 临时阻挡区域类型
TEMPORARY_BLOCK_TYPE_POLYGON = 0	---多边形
TEMPORARY_BLOCK_TYPE_CIRCLE = 1		---圆心

---是否打印debug用的点
PROJECTILE_DEBUG_MODE = default(PROJECTILE_DEBUG_MODE, false)

if ProjectileSystem == nil then
	---@module ProjectileSystem
	ProjectileSystem = class({})

	---@class SurroundGroup 环绕组
	---@field hCaster CDOTA_BaseNPC 环绕的单位
	---@field flAngularVelocity number 角速度
	---@field sGroupName string 组名
	---@field vPosition Vector 当前位置
	---@field flAngle number 角度
	---@field tProjectileIndex table 环绕弹道索引表
	---@field flOffset number z轴偏移
end
...
function public:init(bReload)
	if not bReload then
		...
		---马甲
		self.hProjectileSystemDummy = CreateModifierThinker((IsInToolsMode() and GameRules:GetGameModeEntity() or nil), nil, "modifier_dummy", nil, Vector(0, 0, 0), DOTA_TEAM_NOTEAM, false)
		---存所有临时生成阻挡区域
		self.tTemporaryBlock = {}
		---临时生成阻挡区域索引
		self.iTemporaryBlockIndex = 1
		---存同组的环绕弹道,sGroupName是组名，tProjectileIndex是弹道索引的表
		---@type table<sGroupName, tProjectileIndex>
		self.tGroupSurroundProjectile = {}
		---存环绕弹道组，环绕弹道组有中心位置、角速度、当前方向等相关信息，组里的环绕弹道会根据这些信息排布
		---@type table<string, SurroundGroup>
		self.tSurroundGroup = {}
	end
	---圆心检测半径
	...
	self.hTimer = GameTimer(FrameTime(), function()
		for index, tInfo in pairs(self.tProjectile) do
			if tInfo.iType == PROJECTILE_TYPE_LINEAR then
				xpcall(self._OnLinearProjectileThink, error, self, index)
				-- self:_OnLinearProjectileThink(index)
			elseif tInfo.iType == PROJECTILE_TYPE_TRACKING then
				xpcall(self._OnTrackingProjectileThink, error, self, index)
				-- self:_OnTrackingProjectileThink(index)
			elseif tInfo.iType == PROJECTILE_TYPE_GUIDANCE then
				xpcall(self._OnGuidanceProjectileThink, error, self, index)
				-- self:_OnGuidanceProjectileThink(index)
				-- elseif tInfo.iType == PROJECTILE_TYPE_SURROUND then
				-- 	xpcall(self._OnSurroundProjectileThink, error, index)
				-- self:_OnSurroundProjectileThink(index)
			end
		end
		for sGroupName, tGroupInfo in pairs(self.tSurroundGroup) do
			xpcall(self._OnSurroundGroupThink, error, self, tGroupInfo)
			-- self:_OnSurroundGroupThink(tGroupInfo)
		end
		return FrameTime()
	end)
end
...
--------------------------------------------------------------------------------
---创建线性弹道
---@param tInfo.hAbility 技能 可选
---@param tInfo.hCaster 来源 必填
---@param tInfo.sEffectName 特效 可选，对于非常规弹道特效，不填sEffectName，自行创建特效并绑定到iParticleID即可
---@param tInfo.vSpawnOrigin 起始点 可选，默认从attach_attack1发射
---@param tInfo.vDirection 方向 必填
---@param tInfo.iMoveSpeed 速度 必填
---@param tInfo.flDistance 距离 必填
---@param tInfo.flRadius 半径 可选，会被flStartRadius和flEndRadius覆盖
---@param tInfo.flStartRadius 开始半径 可选
---@param tInfo.flEndRadius 结束半径 可选
---@param tInfo.bHasFrontalCone boolean 可选，是否不去头（选中背后的单位）
---@param tInfo.iUnitTargetTeam 目标队伍 可选，默认调用技能kv设置
---@param tInfo.iUnitTargetType 目标类型 可选，默认调用技能kv设置
---@param tInfo.iUnitTargetFlags 目标标记 可选，默认调用技能kv设置
---@param tInfo.iBounce 弹射次数 可选弹射次数（以下属性该项目暂时不适用）
...
---@param tInfo.iBreakableDamage 对可破坏物造成的伤害 默认1
---@param tInfo.ExtraData 额外数据 其实不需要了，直接在表里填就算是了
---@alias OnProjectileHit fun(hTarget:CDOTA_BaseNPC,vPosition:Vector,tInfo:table):void
---@alias OnProjectileThink fun(vPosition:Vector,tInfo:table):void
---@alias OnProjectileDestroy fun(vPosition:Vector,tInfo:table):void
---@param tInfo {hCaster:CDOTA_BaseNPC,hAbility:CDOTA_Ability_Lua,sEffectName:string,vSpawnOrigin:Vector,vDirection:Vector,iMoveSpeed:number,flDistance:number,flRadius:number,flStartRadius:number,flEndRadius:number,iUnitTargetTeam:number,iUnitTargetType:number,iUnitTargetFlags:number,OnProjectileHit:OnProjectileHit,OnProjectileThink:OnProjectileThink,OnProjectileDestroy:OnProjectileDestroy}
---@return number 弹道索引
function public:CreateLinearProjectile(tInfo)
	tInfo.vPosition = default(tInfo.vSpawnOrigin, tInfo.hCaster:GetAttachmentPosition("attach_attack1"))					---记录当前弹道位置
	tInfo.vDirection.z = 0
	tInfo.vDirection = tInfo.vDirection:Normalized()
	...
	tInfo.iBreakableDamage = default(tInfo.iBreakableDamage, 1)		---对可破坏物造成的伤害
	...
	tInfo.iUnitTargetTeam = default(tInfo.iUnitTargetTeam, (tInfo.hAbility and tInfo.hAbility:GetAbilityTargetTeam() or nil), DOTA_UNIT_TARGET_TEAM_ENEMY)
	tInfo.iUnitTargetType = default(tInfo.iUnitTargetType, (tInfo.hAbility and tInfo.hAbility:GetAbilityTargetType() or nil), DOTA_UNIT_TARGET_BASIC + DOTA_UNIT_TARGET_HERO)
	tInfo.iUnitTargetFlags = default(tInfo.iUnitTargetFlags, (tInfo.hAbility and tInfo.hAbility:GetAbilityTargetFlags() or nil), DOTA_UNIT_TARGET_FLAG_NONE)
...
---@param tInfo.hAbility 技能 可选
---@param tInfo.hCaster 来源 必填
---@param tInfo.hTarget 目标 必填
---@param tInfo.sEffectName 特效 可选，对于非常规弹道特效，不填sEffectName，自行创建特效并绑定到iParticleID即可
---@param tInfo.vSpawnOrigin 起始点 可选，默认从attach_attack1发射
---@param tInfo.iMoveSpeed 速度
---@param tInfo.flRadius 半径 可选，默认是0，不会命中途中经过的单位
---@param tInfo.bHasFrontalCone boolean 可选，是否不去头（选中背后的单位）
---@param tInfo.iUnitTargetTeam 目标队伍 可选，默认调用技能kv设置
---@param tInfo.iUnitTargetType 目标类型 可选，默认调用技能kv设置
---@param tInfo.iUnitTargetFlags 目标标记 可选，默认调用技能kv设置
---@param tInfo.iBounce 弹射次数 可选（以下属性该项目暂时不适用）
---@param tInfo.bCounter 是否能反弹
---@param tInfo.iHitCount 命中次数 默认不计算
---@param tInfo.bIgnoreBlock 是否无视障碍物
---@param tInfo.iBreakableDamage 对可破坏物造成的伤害 默认1
...
---@param tInfo.hTarget 目标 可选，hTarget和vTarget二选一
---@param tInfo.vTarget 目标点 可选，hTarget和vTarget二选一
...
function public:CreateGuidanceProjectile(tInfo)
	...
	tInfo.vTarget = default(tInfo.vTarget, tInfo.hTarget:GetAbsOrigin(), vec3_zero)
	...
	tInfo.flLifeTime = self:_GetLifeTime(tInfo)		---弹道的生命周期
	tInfo.flLifeTimeRemaining = tInfo.flLifeTime			---弹道的剩余生命
	tInfo.iBreakableDamage = default(tInfo.iBreakableDamage, 1)		---对可破坏物造成的伤害
...
---@param tInfo {hCaster:CDOTA_BaseNPC,hAbility:CDOTA_Ability_Lua,sGroup:string,flOffset:number,flCircleRadius:number,flAngularVelocity:number,iCount:number,flLifeTime:number,flRadius:number,iUnitTargetTeam:number,iUnitTargetType:number,iUnitTargetFlags:number,OnProjectileHit:OnProjectileHit,OnProjectileThink:OnProjectileThink,OnProjectileDestroy:OnProjectileDestroy,OnProjectileBounceStart:OnProjectileBounceStart,OnProjectileBounceEnd:OnProjectileBounceEnd,bHasBonusCount:boolean}
function public:CreateGroupSurroundProjectile(tInfo)
	tInfo.bHasBonusCount = default(tInfo.bHasBonusCount, true)
	local iCount = default(tInfo.iCount, 1)
	tInfo.sGroupName = default(tInfo.sGroupName, (tInfo.hAbility and tInfo.hAbility:GetAbilityName() or nil), DoUniqueString("SurroundProjectile"))
	local tProjectileIndex = {}
	local tSurroundGroup = self:_CreateSurroundGroup({
		hCaster = tInfo.hCaster,
		flAngularVelocity = tInfo.flAngularVelocity,
		sGroupName = tInfo.sGroupName,
		flOffset = default(tInfo.flOffset, 0),
		flCircleRadius = default(tInfo.flCircleRadius, 0),
	})
	for i = 1, iCount do
		local _tInfo = shallowcopy(tInfo)
		if #tSurroundGroup.tProjectileIndex == 0 then
			_tInfo.flAngle = 360 / iCount * i
		end
		local iProjectileIndex = ProjectileSystem:CreateSurroundProjectile(_tInfo)
		table.insert(tProjectileIndex, iProjectileIndex)
		-- table.insert(tSurroundGroup.tProjectileIndex, iProjectileIndex)
	end
	tSurroundGroup.tProjectileIndex = ArrayConcat(tSurroundGroup.tProjectileIndex, tProjectileIndex)
	return tProjectileIndex
end

---@alias OnProjectileHit fun(hTarget:CDOTA_BaseNPC,vLocation: Vector,tInfo:table):void
---@alias OnProjectileThink fun(vLocation: Vector,tInfo:table):void
---@alias OnProjectileDestroy fun(vLocation: Vector,tInfo:table):void
---@alias OnProjectileBounceStart fun(vLocation: Vector,tInfo:table):void
---@alias OnProjectileBounceEnd fun(vLocation: Vector,tInfo:table):void
---
---创建环绕弹道（有更复杂需求使用这个api一个个创建环绕弹道，比如一条直线的环绕弹道）
---@param tInfo {hCaster:CDOTA_BaseNPC,hAbility:CDOTA_Ability_Lua,sGroupName:string,flCircleRadius:number,flAngularVelocity:number,flAngle:number,flLifeTime:number,flRadius:number,iUnitTargetTeam:number,iUnitTargetType:number,iUnitTargetFlags:number,OnProjectileHit:OnProjectileHit,OnProjectileThink:OnProjectileThink,OnProjectileDestroy:OnProjectileDestroy,OnProjectileBounceStart:OnProjectileBounceStart,OnProjectileBounceEnd:OnProjectileBounceEnd}
function public:CreateSurroundProjectile(tInfo)
	tInfo.flCircleRadius = default(tInfo.flCircleRadius, 150)
	tInfo.flAngularVelocity = default(tInfo.flAngularVelocity, 90)
	-- tInfo.flAngle = default(tInfo.flAngle, 0)
	tInfo.flLifeTime = tInfo.flLifeTime		---弹道的生命周期
	tInfo.flLifeTimeRemaining = tInfo.flLifeTime			---弹道的剩余生命
	tInfo.flRadius = default(tInfo.flRadius, 0)	---弹道的碰撞半径
	tInfo._PriviousRadius = tInfo.flRadius
	tInfo._NextRadius = tInfo.flRadius
	---环绕弹道的组，相同组的弹道会绕着一个轨道运动
	tInfo.sGroupName = default(tInfo.sGroupName, (tInfo.hAbility and tInfo.hAbility:GetAbilityName() or nil), DoUniqueString("SurroundProjectile"))
	local tGroupInfo = self.tSurroundGroup[tInfo.sGroupName]
	local flCircleRadius = tInfo.flCircleRadius
	local iProjectileCount = #tGroupInfo.tProjectileIndex + 1
	local flAngleInterval = 360 / iProjectileCount
	local flAngle = tGroupInfo.flAngle + flAngleInterval * #tGroupInfo.tProjectileIndex
	if tInfo.flAngle == nil then
		tInfo.flAngle = flAngle
	end
	tInfo.vPosition = tGroupInfo.vPosition + AnglesToVector(QAngle(0, flAngle, 0)) * flCircleRadius
	-- DebugDrawCircle(tInfo.vPosition, Vector(0, 255, 0), 0, 50, true, 1)
	tInfo.iUnitTargetTeam = default(tInfo.iUnitTargetTeam, (tInfo.hAbility and tInfo.hAbility:GetAbilityTargetTeam() or nil), DOTA_UNIT_TARGET_TEAM_NONE)
	tInfo.iUnitTargetType = default(tInfo.iUnitTargetType, (tInfo.hAbility and tInfo.hAbility:GetAbilityTargetType() or nil), DOTA_UNIT_TARGET_NONE)
	tInfo.iUnitTargetFlags = default(tInfo.iUnitTargetFlags, (tInfo.hAbility and tInfo.hAbility:GetAbilityTargetFlags() or nil), DOTA_UNIT_TARGET_FLAG_NONE)
	tInfo.iType = PROJECTILE_TYPE_SURROUND
	tInfo.tTargets = {}	---记录击中过的单位
	tInfo.hThinker = SpawnEntityFromTableSynchronous("prop_dynamic", { origin = tInfo.vPosition, model = "models/development/invisiblebox.vmdl" })
	-- tInfo.hThinker = CreateUnitByName("npc_dota_dummy", tInfo.vPosition, false, tInfo.hCaster, tInfo.hCaster, tInfo.hCaster:GetTeamNumber())
	tInfo.iParticleID = ParticleManager:CreateParticle(tInfo.sEffectName, PATTACH_ABSORIGIN_FOLLOW, tInfo.hThinker)
	-- local iProjectileIndex = self:_InitProjectile(tInfo)
	-- self.tGroupSurroundProjectile[tInfo.sGroup] = self.tGroupSurroundProjectile[tInfo.sGroup] or {}
	-- table.insert(self.tGroupSurroundProjectile[tInfo.sGroup], iProjectileIndex)
	return self:_InitProjectile(tInfo)
end
...
---销毁环绕弹道组
function public:DestroySurroundProjectileWithGroup(sGroupName)
	if self.tGroupSurroundProjectile[sGroupName] == nil then
		return
	end
	for i = #self.tGroupSurroundProjectile[sGroupName], 1, -1 do
		self:_DestroyProjectile(self.tGroupSurroundProjectile[sGroupName][i])
	end
	self:UpdateSurroundProjectileWithGroup(sGroupName)
end

---销毁部分环绕弹道（传入弹道索引的表）
function public:DestroyPartOfSurroundProjectileWithGroup(sGroupName, tGroup)
	for i = #tGroup, 1, -1 do
		ProjectileSystem:DestroyProjectile(tGroup[i], true)
	end
	-- self:UpdateSurroundProjectileWithGroup(sGroupName)
end

---更新环绕弹道
function public:UpdateSurroundProjectileWithGroup(sGroupName)
	if self.tGroupSurroundProjectile[sGroupName] == nil then
		return
	end
	if self.tGroupSurroundProjectile[sGroupName] and #self.tGroupSurroundProjectile[sGroupName] > 0 then
		local flInitAngle = self:_GetProjectileInfo(self.tGroupSurroundProjectile[sGroupName][1]).flAngle
		local flAngleInterval = 360 / #self.tGroupSurroundProjectile[sGroupName]
		for i, v in ipairs(self.tGroupSurroundProjectile[sGroupName]) do
			self:_ChangeSurroundProjectileAngle(v, flInitAngle + flAngleInterval * i)
		end
	end
end
...
function public:IsValidPosition(vPosition)
	local bValid = true
	if not GridNav:IsTraversable(vPosition) or GridNav:IsBlocked(vPosition) or self:IsPositionInTemporaryBlock(vPosition) then
		bValid = false
	end
	return bValid
end
...
---判断点是否在临时阻挡区域里
function public:IsPositionInTemporaryBlock(vPosition)
	local bEnter = false
	for i, tTemporaryBlockData in pairs(self.tTemporaryBlock) do
		if tTemporaryBlockData.iType == TEMPORARY_BLOCK_TYPE_CIRCLE and (tTemporaryBlockData.vCenter - vPosition):Length2D() <= tTemporaryBlockData.flRadius then
			bEnter = true
		end
		if tTemporaryBlockData.iType == TEMPORARY_BLOCK_TYPE_POLYGON and IsPointInPolygon(vPosition, tTemporaryBlockData.tPoints) then
			bEnter = true
		end
	end
	return bEnter
...
---获取点所处的临时阻挡区域
function public:GetTemporaryBlocksInPosition(vPosition)
	local tTemporaryBlocks = {}
	for i, tTemporaryBlockData in pairs(self.tTemporaryBlock) do
		if tTemporaryBlockData.iType == TEMPORARY_BLOCK_TYPE_CIRCLE and (tTemporaryBlockData.vCenter - vPosition):Length2D() <= tTemporaryBlockData.flRadius then
			table.insert(tTemporaryBlocks, tTemporaryBlockData)
		end
		if tTemporaryBlockData.iType == TEMPORARY_BLOCK_TYPE_POLYGON and IsPointInPolygon(vPosition, tTemporaryBlockData.tPoints) then
			table.insert(tTemporaryBlocks, tTemporaryBlockData)
		end
	end
	return tTemporaryBlocks
end
...
---创建一块临时阻挡弹道区域
---@param tBlockData {iType: TEMPORARY_BLOCK_TYPE_CIRCLE | TEMPORARY_BLOCK_TYPE_POLYGON, vCenter: Vector, flRadius: number, tPoints: table, callback: fun(iProjectileIndex: number):void}
---@return iTemporaryBlockIndex
function public:CreateTemporaryBlock(tBlockData, flDuration)
	local iTemporaryBlockIndex = self.iTemporaryBlockIndex
	self.tTemporaryBlock[iTemporaryBlockIndex] = tBlockData
	if flDuration then
		GameTimer(flDuration, function()
			self.tTemporaryBlock[iTemporaryBlockIndex] = nil
		end)
	end
	self.iTemporaryBlockIndex = iTemporaryBlockIndex + 1
	return iTemporaryBlockIndex
end

---移除临时阻挡弹道区域
---@param iTemporaryBlockIndex 索引
function public:RemoveTemporaryBlock(iTemporaryBlockIndex)
	self.tTemporaryBlock[iTemporaryBlockIndex] = nil
end

function public:ProjectileDodge(hUnit)
	-- TODO:
...
function public:GetDirection(iProjectileIndex)
	local vDirection = self:_GetProjectileProperty(iProjectileIndex, "vDirection")
	vDirection.z = 0
	return vDirection
end
---获取弹道技能
...
---获取弹道目标
function public:GetTargetUnit(iProjectileIndex)
	return self:_GetProjectileProperty(iProjectileIndex, "hTarget")
end
---获取弹道目标位置
function public:GetTargetPosition(iProjectileIndex)
	return self:_GetProjectileProperty(iProjectileIndex, "vTarget")
end
---获取弹道目标位置
function public:GetParticle(iProjectileIndex)
	return self:_GetProjectileProperty(iProjectileIndex, "iParticleID")
end
---获取弹道马甲实体
function public:GetThinker(iProjectileIndex)
	return self:_GetProjectileProperty(iProjectileIndex, "hThinker")
end
---获取弹道当前位置
function public:GetPosition(iProjectileIndex)
	return self:_GetProjectileProperty(iProjectileIndex, "vPosition")
end
----------------------------------------Set----------------------------------------
---设置弹道目标
function public:SetTargetUnit(iProjectileIndex, hTarget)
	return self:_SetProjectileProperty(iProjectileIndex, "hTarget", hTarget)
end
---设置弹道目标位置
function public:SetTargetPosition(iProjectileIndex, vTarget)
	return self:_SetProjectileProperty(iProjectileIndex, "vTarget", vTarget)
end
---设置弹道目标位置
function public:SetAngularVelocity(iProjectileIndex, flVelocity)
	return self:_SetProjectileProperty(iProjectileIndex, "flAngularVelocity", flVelocity)
end
...
---@return table
---@private
function public:_FindUnitInLine(iTeamNumber, vStart, vEnd, flStartRadius, flEndRadius, iTargetTeam, iTargetType, iTargetFlags, tInfo)
	...
	for _, hUnit in ipairs(tTargets) do
		...
		if not default(tInfo.bHasFrontalCone, true) and (vUnitLoc - vStart):Length2D() < flStartRadius then
			table.insert(tTargetInLine, hUnit)
		end
		if (vUnitLoc - vEnd):Length2D() < flEndRadius and vDir:Dot((vUnitLoc - vStart):Normalized()) < 0 then
			table.insert(tTargetInLine, hUnit)
		end
		if IsPointInPolygon(vUnitLoc, tPoints) then
			table.insert(tTargetInLine, hUnit)
		end
	end
...
function public:_OnLinearProjectileThink(index)
	...
	if tInfo.bIgnoreBlock == nil or tInfo.bIgnoreBlock == false then
		...
		if not self:IsValidPosition(tInfo.vPosition) or bBlock == true then
			...
			-- 插入判断是否在临时区域内
			local tTemporaryBlocks = self:GetTemporaryBlocksInPosition(tInfo.vPosition)
			for i, tTemporaryBlockData in ipairs(tTemporaryBlocks) do
				if tTemporaryBlockData.callback then
					tTemporaryBlockData.callback(tInfo)
				end
			end
...
function public:_OnTrackingProjectileThink(index)
	...
	tInfo.vTarget = (tInfo.iType == PROJECTILE_TYPE_TRACKING and IsValid(tInfo.hTarget)) and tInfo.hTarget:GetAttachmentPosition("attach_hitloc") or tInfo.vTarget
...
---计算环绕弹道
---@param tGroupInfo SurroundGroup
---@private
function public:_OnSurroundProjectileThink(iProjectileIndex, iGroupIndex, tGroupInfo)
	local tInfo = self.tProjectile[iProjectileIndex]
	if not IsValid(tInfo.hCaster) then
		self:_DestroyProjectile(iProjectileIndex)
		return
	end
	tInfo.vPrevious = tInfo.vPosition	---上一个位置
	-- tInfo.vPrevious.z = tInfo.vSpawnOrigin.z
	local iProjectileCount = #tGroupInfo.tProjectileIndex
	local flAngleInterval = 360 / iProjectileCount
	local flTargetAngle = tGroupInfo.flAngle + flAngleInterval * (iGroupIndex - 1)
	local flAngleDiff = Round(AngleDiff(flTargetAngle, tInfo.flAngle), 2)
	local flAngularVelocity = Round(tGroupInfo.flAngularVelocity * FrameTime(), 2)
	tInfo.flAngle = tInfo.flAngle + (tGroupInfo.flAngularVelocity + flAngleDiff) * FrameTime()
	tInfo.flAngle = tInfo.flAngle % 360
	if math.abs(flAngleDiff) > flAngularVelocity then
	else
		-- tInfo.flAngle = flTargetAngle
	end
	-- local radius = math.min(math.abs(AngleDiff(350, tInfo.flAngle)), 30) * math.min(math.abs(AngleDiff(350, tInfo.flAngle)), 30) / -8 + 112.5
	local flCircleRadius = tInfo.flCircleRadius
	local flTrackRadius = 0
	-- if tGroupInfo.hCaster:IsTrackingSurround() then
	-- 	for i, hUnit in ipairs(tGroupInfo.tTrackTargets) do
	-- 		local c = (hUnit:GetAbsOrigin() - tGroupInfo.vPosition):Length2D() - flCircleRadius
	-- 		local a = -1 / 900 * c
	-- 		local b = (900 * a + c) / 30
	-- 		local x = math.min(math.abs(AngleDiff(VectorToAngles((hUnit:GetAbsOrigin() - tGroupInfo.vPosition):Normalized()).y, tInfo.flAngle)), 30)
	-- 		flTrackRadius = flTrackRadius + a * x * x + b * x + c
	-- 	end
	-- 	flTrackRadius = math.min(flTrackRadius, 150)
	-- end
	flCircleRadius = flCircleRadius + flTrackRadius
	tInfo.vPosition = tGroupInfo.vPosition + AnglesToVector(QAngle(0, tInfo.flAngle, 0)) * flCircleRadius
	tInfo.hThinker:SetAbsOrigin(tInfo.vPosition)
	-- think
	self:_OnProjectileThink(iProjectileIndex)
	-- 命中单位
	if not self:_OnProjectileHit(iProjectileIndex) then
		return
	end
	-- 计算生命周期，销毁
	if tInfo.flLifeTimeRemaining then
		tInfo.flLifeTimeRemaining = tInfo.flLifeTimeRemaining - FrameTime()
		if tInfo.flLifeTimeRemaining <= 0 then
			self:_DestroyProjectile(iProjectileIndex)
		end
...
---计算环绕组
---@param tGroupInfo SurroundGroup
function public:_OnSurroundGroupThink(tGroupInfo)
	if not IsValid(tGroupInfo.hCaster) then
		for iGroupIndex, iProjectileIndex in ipairs(tGroupInfo.tProjectileIndex) do
			self:_OnSurroundProjectileThink(iProjectileIndex, iGroupIndex, tGroupInfo)
		end
		self:_DestroySurroundGroup(tGroupInfo.sGroupName)
		return
	end
	local flAngularVelocity = tGroupInfo.flAngularVelocity
	tGroupInfo.vPosition = tGroupInfo.hCaster:GetAbsOrigin() + Vector(0, 0, tGroupInfo.flOffset)
	tGroupInfo.flAngle = tGroupInfo.flAngle + flAngularVelocity * FrameTime()
	tGroupInfo.flAngle = tGroupInfo.flAngle % 360
	local flCircleRadius = (tGroupInfo.flCircleRadius)
	local iProjectileCount = #tGroupInfo.tProjectileIndex
	local flAngleInterval = 360 / iProjectileCount
	if false then
		-- if tGroupInfo.hCaster:IsTrackingSurround() then
		local tOuterTargets = FindUnitsInRadius(tGroupInfo.hCaster:GetTeamNumber(), tGroupInfo.vPosition, nil, flCircleRadius + 160, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_BASIC + DOTA_UNIT_TARGET_HERO, DOTA_UNIT_TARGET_FLAG_NONE, FIND_CLOSEST, false)
		local tInnerTargets = FindUnitsInRadius(tGroupInfo.hCaster:GetTeamNumber(), tGroupInfo.vPosition, nil, flCircleRadius - 160, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_BASIC + DOTA_UNIT_TARGET_HERO, DOTA_UNIT_TARGET_FLAG_NONE, FIND_CLOSEST, false)
		for i, hUnit in ipairs(tInnerTargets) do
			ArrayRemove(tOuterTargets, hUnit)
		end
		tGroupInfo.tTrackTargets = tOuterTargets
	end
	for iGroupIndex, iProjectileIndex in ipairs(tGroupInfo.tProjectileIndex) do
		self:_OnSurroundProjectileThink(iProjectileIndex, iGroupIndex, tGroupInfo)
	end
...
function public:_OnProjectileHit(iProjectileIndex, tDefaultTargets)
	...
	if tInfo == nil then return true end
	if not IsValid(tInfo.hCaster) then return true end
	-- if tInfo._PriviousRadius == 0 and tInfo._NextRadius == 0 then return true end
	local bDestroy = false
	-- 命中单位
	local tTargets = self:_FindUnitInLine(tInfo.hCaster:GetTeamNumber(), tInfo.vPrevious, tInfo.vPosition, tInfo._PriviousRadius, tInfo._NextRadius, tInfo.iUnitTargetTeam, tInfo.iUnitTargetType, tInfo.iUnitTargetFlags, tInfo)
	...
	for _, hUnit in ipairs(tTargets) do
		if vlua.find(tInfo.tTargets, hUnit) == nil then
			if tInfo.funcUnitFilter == nil or tInfo.funcUnitFilter(hUnit) == true then
				if hUnit:IsFriendly(tInfo.hCaster) then
					-- 友军就必中
					...
					-- local iEvasion = GetEvasion(hUnit, {})
					-- if RollPercentage(iEvasion) then
					-- 	SendOverheadEventMessage(hUnit:GetPlayerOwner(), OVERHEAD_ALERT_EVADE, hUnit, 0, hUnit:GetPlayerOwner())
					-- else
					-- end
					if tInfo.OnProjectileHit then
						xpcall(tInfo.OnProjectileHit, error, hUnit, tInfo.vPosition, tInfo)
...
function public:_InitProjectile(tInfo)
	...
	tInfo.iProjectileIndex = iProjectileIndex
	-- 递增index
...
function public:_GetBounce(tInfo)
	tInfo._iOriginalBounce = (tInfo.iBounce or 0)
	return tInfo._iOriginalBounce
end
---获取弹道距离
---@private
function public:_GetDistance(tInfo)
	return tInfo.flDistance
end
---获取制导弹道生命周期
---@private
function public:_GetLifeTime(tInfo)
	return default(tInfo.flLifeTime, PROJECTILE_GUIDANCE_MAX_LIFETIME)
end
---获取弹道速度
---@private
function public:_GetMoveSpeed(tInfo)
	return tInfo.iMoveSpeed
end
---计算路径上有没有碰撞物
---@private
...
---@private
function public:_SetProjectileProperty(iProjectileIndex, sKey, value)
	local tInfo = self:_GetProjectileInfo(iProjectileIndex)
	tInfo[sKey] = value
end
...
function public:_DestroyProjectile(iProjectileIndex)
	...
	if tInfo.iParticleID and tInfo.iParticleID ~= -1 then
		ParticleManager:DestroyParticle(tInfo.iParticleID, false)
	end
	...
	---销毁环绕弹道的记录
	if tInfo.iType == PROJECTILE_TYPE_SURROUND then
		ArrayRemove(self.tSurroundGroup[tInfo.sGroupName].tProjectileIndex, iProjectileIndex)
		-- ArrayRemove(self.tGroupSurroundProjectile[tInfo.sGroup], iProjectileIndex)
		---如果组里面为空，销毁该组
		if #self.tSurroundGroup[tInfo.sGroupName].tProjectileIndex == 0 then
			-- self.tGroupSurroundProjectile[tInfo.sGroupName] = nil
			self:_DestroySurroundGroup(tInfo.sGroupName)
		end
	end
...
---更改环绕弹道的角度
---@private
function public:_ChangeSurroundProjectileAngle(iProjectileIndex, flAngle)
	local tInfo = self:_GetProjectileInfo(iProjectileIndex)
	tInfo.flAngle = flAngle
	-- local flInterval = tInfo.flAngle - flAngle
	-- if flInterval < 0 then
	-- 	flInterval = 360 + flInterval
	-- end
	-- local flAngularVelocity = flInterval / 0.3
	-- tInfo.flAngularVelocity = tInfo.flAngularVelocity - flAngularVelocity
	-- GameTimer(0.3, function()
	-- 	tInfo.flAngularVelocity = tInfo.flAngularVelocity + flAngularVelocity
	-- end)
end
---创建一个环绕弹道组
---@param tSurroundGroup SurroundGroup
---@return SurroundGroup
---@private
function public:_CreateSurroundGroup(tSurroundGroup)
	if self.tSurroundGroup[tSurroundGroup.sGroupName] == nil then
		tSurroundGroup.flAngle = 0
		tSurroundGroup.flCircleRadius = tSurroundGroup.flCircleRadius
		tSurroundGroup.tProjectileIndex = {}
		tSurroundGroup.vPosition = tSurroundGroup.hCaster:GetAbsOrigin()
		self.tSurroundGroup[tSurroundGroup.sGroupName] = tSurroundGroup
	end
	return self.tSurroundGroup[tSurroundGroup.sGroupName]
end
---删除环绕弹道组
---@param sGroupName string
---@private
function public:_DestroySurroundGroup(sGroupName)
	self.tSurroundGroup[sGroupName] = nil
end

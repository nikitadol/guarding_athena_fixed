		...
		for i = 1, 6 do
			...
			if hItem then
				...
				if string.sub(sItemName, 6, 10) == "salve" then
					...
					local flHealAmount = hItem:GetSpecialValueFor("hp_per_tick") * hItem:GetSpecialValueFor("buff_duration")
					if hHero:GetHealthDeficit() * 2 >= flHealAmount and not hHero:HasModifier("modifier_item_salve" .. iLevel) then
						ExecuteOrder(hParent, DOTA_UNIT_ORDER_CAST_NO_TARGET, hItem)
					end
				end
			end
		end
		for i = 1, 6 do
			...
			if hItem then
				...
				if string.sub(sItemName, 6, 12) == "clarity" then
					...
					local flHealAmount = hItem:GetSpecialValueFor("mana_per_tick") * hItem:GetSpecialValueFor("buff_duration")

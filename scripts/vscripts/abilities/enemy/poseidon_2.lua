---@class poseidon_2: eom_ability
poseidon_2 = eom_ability({})
...
----------------------------------------Modifier----------------------------------------
---@class modifier_poseidon_2 : eom_modifier
modifier_poseidon_2 = eom_modifier({
	Name = "modifier_poseidon_2",
	IsHidden = true,
	IsDebuff = false,
	IsPurgable = false,
...
function modifier_poseidon_2:OnAttackRecord(params)
	if not IsValid(params.target) or params.target:GetClassname() == "dota_item_drop" then return end
	if params.attacker == self:GetParent() and not AttackFilter(params.record, ATTACK_STATE_NOT_USECASTATTACKORB, ATTACK_STATE_NO_CLEAVE, ATTACK_STATE_NO_EXTENDATTACK) then
				...
				DoCleaveAction(params.attacker, params.target, self.cleave_starting_width, self.cleave_ending_width, self.cleave_distance, function(hTarget)
					if IsValid(hTarget) then
						if not params.attacker:IsIllusion() then
							params.attacker:DealDamage(hTarget, self:GetAbility(), params.original_damage * cleave_damage * 0.01, DAMAGE_TYPE_PURE, DOTA_DAMAGE_FLAG_NO_SPELL_AMPLIFICATION + DOTA_DAMAGE_FLAG_NO_SPELL_LIFESTEAL + DOTA_DAMAGE_FLAG_USE_COMBAT_PROFICIENCY)
						end
...
----------------------------------------Modifier----------------------------------------
---@class modifier_poseidon_2_check : eom_modifier
modifier_poseidon_2_check = eom_modifier({
	Name = "modifier_poseidon_2_check",
	IsHidden = true,
	IsDebuff = false,
	IsPurgable = false,
	IsPurgeException = false,
...
----------------------------------------Modifier----------------------------------------
---@class modifier_poseidon_2_animation : eom_modifier
modifier_poseidon_2_animation = eom_modifier({
	Name = "modifier_poseidon_2_animation",
	IsHidden = true,
	IsDebuff = false,
	IsPurgable = false,
	IsPurgeException = false,

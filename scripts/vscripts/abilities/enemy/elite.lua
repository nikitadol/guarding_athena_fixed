...
function modifier_elite:OnCreated(params)
	...
	if IsServer() then
		...
		hParent:SetMinimumGoldBounty(hParent:GetGoldBounty() * (Spawner.difficulty / 2) * Spawner.unitFactor.eliteFactor ^ Rounds:GetRoundNumber())
		hParent:SetMaximumGoldBounty(hParent:GetGoldBounty() * (Spawner.difficulty / 2) * Spawner.unitFactor.eliteFactor ^ Rounds:GetRoundNumber())
		hParent:SetBaseDamageMin(hParent:GetBaseDamageMin() * (Spawner.difficulty / 2) * Spawner.unitFactor.eliteFactor ^ Rounds:GetRoundNumber())
		hParent:SetBaseDamageMax(hParent:GetBaseDamageMax() * (Spawner.difficulty / 2) * Spawner.unitFactor.eliteFactor ^ Rounds:GetRoundNumber())
		hParent:SetBaseMaxHealth(hParent:GetBaseMaxHealth() * (Spawner.difficulty / 2) * Spawner.unitFactor.eliteFactor ^ Rounds:GetRoundNumber())

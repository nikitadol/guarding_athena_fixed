...
if ability_base_ai == nil then
	ability_base_ai = class({
		-- iBehavior = DOTA_ABILITY_BEHAVIOR_NO_TARGET,	-- 技能行为，主动技能会自动读取kv技能行为:DOTA_ABILITY_BEHAVIOR_UNIT_TARGET,DOTA_ABILITY_BEHAVIOR_POINT
		iSearchBehavior = AI_SEARCH_BEHAVIOR_NONE, -- 技能AI搜寻行为
		-- iAOERadius = self:GetRadius(),	-- AOE技能搜寻范围，可以指定为指定值或者自动读取GetRadius的值，无目标技能不填则cd好了就放，填值则判断有单位就放
		-- iStartWidth = self:GetStartWidth(),	-- 直线技能搜寻开始范围，可以指定为指定值或者自动读取GetStartWidth的值
		-- iEndWidth = self:GetEndWidth(),	-- 直线技能搜寻结束范围，可以指定为指定值或者自动读取GetEndWidth的值
...
function ability_base_ai:GetRadius()
	if self.iAOERadius then
		return self.iAOERadius
	end
	return 0
end
function ability_base_ai:GetStartWidth()
	if self.iStartWidth then
		return self.iStartWidth
	end
	return 0
end
function ability_base_ai:GetEndWidth()
...
function ability_base_ai:Spawn()
	if IsServer() then
		...
		if self.iTargetTeam == 0 then
			self.iTargetTeam = DOTA_UNIT_TARGET_TEAM_ENEMY
		end
		if self.iTargetType == 0 then
		...
		if self.iBehavior == DOTA_ABILITY_BEHAVIOR_NO_TARGET then	-- 无目标技能
			self:GameTimer(0, function()
				if self:IsAbilityReady() then
					...
					local iAOERadius = self:GetRadius()
					if iAOERadius > 0 then
						...
						if self.funcSortFunction then
							table.sort(tTargets, self.funcSortFunction)
						end
		...
		elseif self.iBehavior == DOTA_ABILITY_BEHAVIOR_POINT then	-- 点目标技能
			self:GameTimer(0, function()
				if self:IsAbilityReady() then
					...
					local iAOERadius = self:GetRadius()
					local iStartWidth = self:GetStartWidth()
					local iEndWidth = self:GetEndWidth()
...
function ability_base_ai:CastAbilityOnPosition(vPosition)
	ExecuteOrder(self:GetCaster(), DOTA_UNIT_ORDER_CAST_POSITION, self, vPosition)
end
-- 单位目标技能施法
function ability_base_ai:CastAbilityOnTarget(hTarget)
	ExecuteOrder(self:GetCaster(), DOTA_UNIT_ORDER_CAST_TARGET, self, hTarget)
end
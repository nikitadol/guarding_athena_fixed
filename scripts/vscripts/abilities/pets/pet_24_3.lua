---@class pet_24_3: eom_ability
pet_24_3 = eom_ability({})
function pet_24_3:GetIntrinsicModifierName()
	return "modifier_pet_24_3"
end
----------------------------------------Modifier----------------------------------------
---@class modifier_pet_24_3 : eom_modifier
modifier_pet_24_3 = eom_modifier({
	Name = "modifier_pet_24_3",
	IsHidden = true,
	IsDebuff = false,
	IsPurgable = false,
...
function modifier_pet_24_3:OnIntervalThink()
	if self:GetCaster().GetMaster ~= nil and not self:GetCaster():GetMaster():HasModifier("modifier_pet_24_3_buff") then
		self.hModifier = self:GetCaster():GetMaster():AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_pet_24_3_buff", nil)
		self:StartIntervalThink(-1)
	end
end
function modifier_pet_24_3:OnDestroy()
	if IsServer() then
		self.hModifier:Destroy()
	end
end
----------------------------------------Modifier----------------------------------------
---@class modifier_pet_24_3_buff : eom_modifier
modifier_pet_24_3_buff = eom_modifier({
	Name = "modifier_pet_24_3_buff",
	IsHidden = true,
	IsDebuff = false,
	IsPurgable = false,
	IsPurgeException = false,
...
function modifier_pet_24_3_buff:GetAbilitySpecialValue()
	self.distance = self:GetAbilitySpecialValueFor("distance")
end
function modifier_pet_24_3_buff:EDeclareFunctions()
	return {
		MODIFIER_EVENT_ON_ORDER = { self:GetParent() }
	}
end
...
----------------------------------------Modifier----------------------------------------
---@class modifier_pet_24_3_jump : eom_modifier
modifier_pet_24_3_jump = eom_modifier({
	Name = "modifier_pet_24_3_jump",
	IsHidden = false,
	IsDebuff = false,
	IsPurgable = false,
	IsPurgeException = false,
...
function modifier_pet_24_3_jump:OnCreated(params)
	if IsServer() then
		if self:ApplyHorizontalMotionController() then
			...
			self.flDamage = self:GetParent():GetPrimaryStats() * self:GetAbilityDamage()
			self.stun_duration = self:GetAbility():GetDuration()

...
function modifier_item_gong:OnAttack(params)
	...
	if params.attacker == self:GetParent() and not AttackFilter(params.record, ATTACK_STATE_NO_EXTENDATTACK) then
		...
		if not params.attacker:IsRangedAttacker() then
			sProjectileName = "particles/econ/attack/attack_modifier_ti9.vpcf"
		end
		local iProjectileSpeed = params.attacker:GetProjectileSpeed()
		if iProjectileSpeed == 0 then
			iProjectileSpeed = 900
		end
		print("bigciba", params.attacker:GetProjectileSpeed())
		...
		for i, hTarget in pairs(tTargets) do
			...
			local info = {
				hCaster = params.attacker,
				hTarget = hTarget,
				sEffectName = sProjectileName,
				hAbility = self:GetAbility(),
				iMoveSpeed = iProjectileSpeed,
				vSpawnOrigin = params.attacker:GetAttachmentPosition("attach_attack1") or params.attacker:GetAbsOrigin(),
				OnProjectileHit = function(_hTarget, vLocation, tInfo)
					params.attacker:DealDamage(_hTarget, self:GetAbility(), self:GetAbilitySpecialValueFor("damage"), DAMAGE_TYPE_MAGICAL)
				end
			}
		...
		if iRemaining > 0 then
			for i = 1, iRemaining do
				...
				local info = {
					...
					hAbility = self:GetAbility(),
					iMoveSpeed = iProjectileSpeed,
					vSpawnOrigin = params.attacker:GetAbsOrigin() + Vector(RandomInt(-200, 200), RandomInt(-200, 200), RandomInt(0, 120)),
					OnProjectileHit = function(_hTarget, vLocation, tInfo)
						params.attacker:DealDamage(_hTarget, self:GetAbility(), self:GetAbilitySpecialValueFor("damage"), DAMAGE_TYPE_MAGICAL)
					end
				}

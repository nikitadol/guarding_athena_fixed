---@class monkey_king_3 : eom_ability
monkey_king_3 = eom_ability({})
	...
	if IsServer() then
		...
		if not hCaster:IsSummoned() then
			...
			for i = 1, iMaxCount do
				...
				local hSoldier = hCaster:SummonUnit(sUnitName, hCaster:GetAbsOrigin(), false, nil, tData)
				...
				hSoldier:AddNewModifier(hSoldier, hSoldier:GetDummyAbility(), "modifier_attribute", nil)
				hSoldier:AddNewModifier(hSoldier, hSoldier:GetDummyAbility(), "modifier_hero_attribute", nil)
...
function monkey_king_3:CreateSoldier(vPosition, flDuration)
	if IsServer() then
		...
		if #self.tFreeSoldiers > 0 then
			...
			hSoldier.tPermanentAttribute = hCaster.tPermanentAttribute
			FindClearSpaceForUnit(hSoldier, vPosition, true)
			...
			for iItemSlot = 0, 5 do
				...
				if hItem ~= nil then
					...
					local hNewItem = CreateItem(sItemName, nil, hSoldier)
			...
			if hCaster:HasModifier("modifier_reborn") then
				hSoldier:AddNewModifier(hSoldier, nil, "modifier_reborn", nil):SetStackCount(hCaster:GetModifierStackCount("modifier_reborn", hCaster))
			end
			if hCaster:HasModifier("modifier_scepter") then
				hSoldier:AddNewModifier(hSoldier, nil, "modifier_scepter", nil)
			end
			...
			hSoldier:Stop()
			ExecuteOrder(hSoldier, DOTA_UNIT_ORDER_ATTACK_MOVE, hSoldier:GetAbsOrigin())
			return hSoldier
		end
	end
end
...
function modifier_monkey_king_3_buff:OnCreated(params)
	local hParent = self:GetParent()
	if IsServer() then
		local iParticleID = ParticleManager:CreateParticle("particles/units/heroes/hero_monkey_king/monkey_king_tap_buff.vpcf", PATTACH_ABSORIGIN, hParent)
...
function modifier_monkey_king_3_soldier:ECheckState()
	return {
...
function modifier_monkey_king_3_activity:OnCreated(params)
	...
	if IsServer() then
		...
		local iParticleID = ParticleManager:CreateParticle("particles/units/heroes/hero_monkey_king/monkey_king_4_position.vpcf", PATTACH_WORLDORIGIN, nil)
		ParticleManager:SetParticleControl(iParticleID, 0, hParent:GetAbsOrigin())
		self:AddParticle(iParticleID, false, false, -1, false, false)
		local iParticleID = ParticleManager:CreateParticle("particles/units/heroes/hero_monkey_king/monkey_king_4_position_c0.vpcf", PATTACH_WORLDORIGIN, nil)
		ParticleManager:SetParticleControl(iParticleID, 0, hParent:GetAbsOrigin())

---@class monkey_king_4 : eom_ability
monkey_king_4 = eom_ability({})
function monkey_king_4:OnSpellStart()
	---@type CDOTA_BaseNPC
	local hCaster = self:GetCaster()
	local vCasterLoc = hCaster:GetAbsOrigin()
	local vPosition = self:GetCursorPosition()
	local thump_damage = self:GetSpecialValueFor("thump_damage")
	...
	self:GameTimer(0.2, function()
		...
		hCaster:EmitSound("Hero_MonkeyKing.Strike.Impact")
	...
	self:GameTimer(1, function()
		if iCount > 0 then
			CreateModifierThinker(hCaster, self, "modifier_monkey_king_4_thinker", { duration = 0.3, crush_damage = crush_damage }, vPosition + RandomVector(RandomInt(0, 100)), hCaster:GetTeamNumber(), false)
			iCount = iCount - 1
			vPosition = vPosition
			return 1
		end
...
function modifier_monkey_king_4_thinker:OnCreated(params)
	if IsServer() then
		self.crush_damage = params.crush_damage
	else
...
function modifier_monkey_king_4_thinker:OnDestroy()
	if IsServer() then
		...
		local flDamage = self.crush_damage

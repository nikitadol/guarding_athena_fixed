---@class monkey_king_2 : eom_ability
monkey_king_2 = eom_ability({})
...
function modifier_monkey_king_2:EDeclareFunctions()
	return {
		EOM_MODIFIER_PROPERTY_INCOMING_DAMAGE_PERCENTAGE = -self.damage_reduce_percent
	}
end
---------------------------------------------------------------------
...
function modifier_monkey_king_2_shield:OnCreated(params)
	if IsServer() then
		self:GetParent():AddShield(params.flShieldHealth, self)
		self.flDamage = params.flShieldHealth
	else
		local hParent = self:GetParent()
...
function modifier_monkey_king_2_shield:OnRefresh(params)
	if IsServer() then
		self.flDamage = self.flDamage + params.flShieldHealth
		self:GetParent():AddShield(params.flShieldHealth, self)
	end
end
...
function modifier_monkey_king_2_shield:OnShieldDestroy()
	if IsServer() then
		self:Destroy()
	end
end
function modifier_monkey_king_2_shield:EDeclareFunctions()
	return {
		-- MODIFIER_PROPERTY_TOTAL_CONSTANT_BLOCK,
		EOM_MODIFIER_PROPERTY_ARMOR_BONUS = self.bonus_armor,
	}
end

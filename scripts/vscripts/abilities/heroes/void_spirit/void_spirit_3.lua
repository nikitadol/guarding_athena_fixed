---@class void_spirit_3 : eom_ability
void_spirit_3 = eom_ability({})
...
function void_spirit_3:AstralStep(vPosition)
	...
	for _, hTarget in pairs(tTargets) do
		--造成攻击
		hCaster:Attack(hTarget, ATTACK_STATE_SKIPCOOLDOWN + ATTACK_STATE_NO_EXTENDATTACK + ATTACK_STATE_NEVERMISS)
		--减速
...
function void_spirit_3:AstralStepScepter(vPosition)
	...
	for _, hTarget in pairs(tTargets) do
		--造成攻击
		hCaster:Attack(hTarget, ATTACK_STATE_NO_EXTENDATTACK + ATTACK_STATE_NEVERMISS)
		--减速
		hTarget:AddNewModifier(hCaster, self, 'modifier_void_spirit_3_debuff', { duration = pop_damage_delay })
	end
	hCaster:RemoveModifierByName("modifier_void_spirit_3")
...
function void_spirit_3:OnScepterLevelup(iLevel)
	local hCaster = self:GetCaster()
	hCaster:AddNewModifier(hCaster, self, "modifier_void_spirit_3_scepter", nil)
end
function void_spirit_3:OnScepterRemove(iLevel)
	local hCaster = self:GetCaster()
	hCaster:RemoveModifierByName("modifier_void_spirit_3_scepter")
end
----------------------------------------Modifier----------------------------------------
---@class modifier_void_spirit_3 : eom_modifier
modifier_void_spirit_3 = eom_modifier({
	Name = "modifier_void_spirit_3",
	IsHidden = true,
	IsDebuff = false,
	IsPurgable = false,
...
function modifier_void_spirit_3:GetAbilitySpecialValue()
	self.bonus_damage = self:GetAbilitySpecialValueFor("bonus_damage")
end
function modifier_void_spirit_3:EDeclareFunctions()
	return {
		EOM_MODIFIER_PROPERTY_ATTACK_DAMAGE_BONUS = self.bonus_damage
	}
end
----------------------------------------Modifier----------------------------------------
---@class modifier_void_spirit_3_debuff : eom_modifier
modifier_void_spirit_3_debuff = eom_modifier({
	Name = "modifier_void_spirit_3_debuff",
	IsHidden = false,
	IsDebuff = true,
	IsPurgable = false,
	IsPurgeException = false,
...
function modifier_void_spirit_3_debuff:GetAbilitySpecialValue()
...
function modifier_void_spirit_3_debuff:OnDestroy()
	if IsServer() then
		local hCaster = self:GetCaster()
		local flDamage = (self.damage * self:GetStackCount()) * (1 + self:GetStackCount() * self.damage_deepen * 0.01)
...
----------------------------------------Modifier----------------------------------------
---@class modifier_void_spirit_3_scepter : eom_modifier
modifier_void_spirit_3_scepter = eom_modifier({
	Name = "modifier_void_spirit_3_scepter",
	IsHidden = true,
	IsDebuff = false,
	IsPurgable = false,
	IsPurgeException = false,
...
function modifier_void_spirit_3_scepter:GetAbilitySpecialValue()
	self.max_travel_distance = self:GetAbilitySpecialValueFor("max_travel_distance")
end
function modifier_void_spirit_3_scepter:EDeclareFunctions()
	return {
		EOM_MODIFIER_PROPERTY_ATTACK_RANGE_BONUS,
		MODIFIER_EVENT_ON_ATTACK_LANDED = { self:GetParent() },
	}
end
function modifier_void_spirit_3_scepter:EOM_GetModifierAttackRangeBonus()
	if self:GetParent():GetScepterLevel() >= 3 then
		return self.max_travel_distance * 0.5
	end
end
function modifier_void_spirit_3_scepter:OnAttackLanded(params)
	if IsServer() then
		if self:GetParent():GetScepterLevel() >= 3 and not AttackFilter(params.record, ATTACK_STATE_NO_EXTENDATTACK) then
			self:GetAbility():AstralStepScepter(params.target:GetAbsOrigin())
		end
	end
end
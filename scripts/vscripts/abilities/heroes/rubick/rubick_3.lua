---@class rubick_3 : eom_ability
rubick_3 = eom_ability({})
...
----------------------------------------Modifier----------------------------------------
---@class modifier_rubick_3_thinker : eom_modifier
modifier_rubick_3_thinker = eom_modifier({
	Name = "modifier_rubick_3_thinker",
	IsHidden = true,
	IsDebuff = false,
	IsPurgable = false,
...
function modifier_rubick_3_thinker:GetAbilitySpecialValue()
...
function modifier_rubick_3_thinker:OnIntervalThink()
	...
	hCaster:DealDamage(tTargets, self:GetAbility(), self.damage)
	-- 天赋
	...
	for _, hUnit in pairs(tTargets) do
		local flDistance = (hParent:GetAbsOrigin() - hUnit:GetAbsOrigin()):Length2D()
		local vDirection = (hParent:GetAbsOrigin() - hUnit:GetAbsOrigin()):Normalized()
		hUnit:SetAbsOrigin(hUnit:GetAbsOrigin() + vDirection * flDistance / 20)
	end
	self:StartIntervalThink(self.interval)
end
----------------------------------------Modifier----------------------------------------
---@class modifier_rubick_3_debuff : eom_modifier
modifier_rubick_3_debuff = eom_modifier({
	Name = "modifier_rubick_3_debuff",
	IsPurgable = false,
	IsPurgeException = false,
	IsStunDebuff = false,
	AllowIllusionDuplicate = true,

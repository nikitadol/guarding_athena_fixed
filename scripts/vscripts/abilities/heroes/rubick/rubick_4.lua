---@class rubick_4 : eom_ability
rubick_4 = eom_ability({})
...
function rubick_4:OnSpellStart()
	...
	if hCaster.HasArcana then
		...
		for _, hUnit in ipairs(tTargets) do
			hCaster:DealDamage(hUnit, self, hUnit:GetCustomMaxHealth() * health_reduce_pct * 0.01)
		end
	end
...
----------------------------------------Modifier----------------------------------------
---@class modifier_rubick_4_thinker : eom_modifier
modifier_rubick_4_thinker = eom_modifier({
	Name = "modifier_rubick_4_thinker",
	IsHidden = true,
	IsDebuff = false,
	IsPurgable = false,
...
function modifier_rubick_4_thinker:GetAbilitySpecialValue()
...
function modifier_rubick_4_thinker:OnIntervalThink()
	local hCaster = self:GetCaster()
	local hParent = self:GetParent()
	local flDamage = self.damage * (1 + self:GetStackCount() * self.damage_deepen * 0.01)
...
----------------------------------------Modifier----------------------------------------
---@class modifier_rubick_4_debuff : eom_modifier
modifier_rubick_4_debuff = eom_modifier({
	Name = "modifier_rubick_4_debuff",
	IsPurgable = false,
	IsPurgeException = false,
	IsStunDebuff = false,
	AllowIllusionDuplicate = true,
})
function modifier_rubick_4_debuff:GetAbilitySpecialValue()
...
function modifier_rubick_4_debuff:OnIntervalThink()
	local hParent = self:GetParent()
	if not IsValid(self:GetAuraOwner()) then
		self:Destroy()
		return
	end

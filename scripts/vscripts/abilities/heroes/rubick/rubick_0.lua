---@class rubick_0 : eom_ability
rubick_0 = eom_ability({})
...
function rubick_0:FadeBolt(hTarget)
	local hCaster = self:GetCaster()
	local flDamage = self:GetSpecialValueFor("damage_factor")
...
function rubick_0:SpaceRift(vPosition)
	...
	for _, hUnit in pairs(tTargets) do
		...
		if (hUnit:GetAbsOrigin() - vPosition):Length2D() <= flCenterRadius then
			_flDamage = _flDamage + hUnit:GetCustomMaxHealth() * flCenterDamage
		end
...
----------------------------------------Modifier----------------------------------------
---@class modifier_rubick_0 : eom_modifier
modifier_rubick_0 = eom_modifier({
	Name = "modifier_rubick_0",
	IsHidden = true,
	IsDebuff = false,
	IsPurgable = false,
...
function modifier_rubick_0:GetAbilitySpecialValue()
...
function modifier_rubick_0:EDeclareFunctions()
	return {

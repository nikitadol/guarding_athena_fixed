---@class rubick_1 : eom_ability
rubick_1 = eom_ability({})
...
----------------------------------------Modifier----------------------------------------
---@class modifier_rubick_1 : eom_modifier
modifier_rubick_1 = eom_modifier({
	Name = "modifier_rubick_1",
	IsHidden = true,
	IsDebuff = false,
	IsPurgable = false,
...
function modifier_rubick_1:GetAbilitySpecialValue()
...
function modifier_rubick_1:OnCreated(params)
	if IsServer() then
		...
		if hCaster.HasArcana then
			...
			self.hModifier = self:GetParent():AddNewModifier(hCaster, self:GetAbility(), "modifier_no_health_bar", { duration = self:GetDuration() })
		end
		...
		-- damage
		local flDamage = self.pull_damage * (1 + params.flUnitCount * self.pull_bonus * 0.01)
...
----------------------------------------Modifier----------------------------------------
---@class modifier_rubick_1_thinker : eom_modifier
modifier_rubick_1_thinker = eom_modifier({
	Name = "modifier_rubick_1_thinker",
	IsHidden = true,
	IsDebuff = false,
	IsPurgable = false,
	IsPurgeException = false,
...
function modifier_rubick_1_thinker:GetAbilitySpecialValue()
...
function modifier_rubick_1_thinker:OnDestroy()
	if IsServer() then
		...
		-- 造成伤害
		local flDamage = self.open_damage * (1 + #tTargets * self.pull_bonus * 0.01)

...
function modifier_lina_4_meteor:OnDestroy()
	if IsServer() then
		...
		local hAbility = self:GetAbility()
		local vPosition = hParent:GetAbsOrigin()
		local flDamage = hAbility:GetSpecialValueFor("damage")
		if hCaster:GetScepterLevel() >= 4 then
			flDamage = flDamage * (1 + hAbility:GetSpecialValueFor("scepter_meteor_damage_pct") * 0.01)
		end

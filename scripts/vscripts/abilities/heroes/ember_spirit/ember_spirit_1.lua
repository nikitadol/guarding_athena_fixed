---@class ember_spirit_1 : eom_ability
ember_spirit_1 = eom_ability({})
...
function ember_spirit_1:OnScepterLevelup(iLevel)
	if iLevel == 4 then
		self:SetMaxCharges(self:GetSpecialValueFor("scepter_bonus_charges"), self:GetSpecialValueFor("scepter_cooldown"))
	end
end
function ember_spirit_1:OnScepterRemove()
...
function modifier_ember_spirit_1:OnCreated(params)
	if IsServer() then
		...
		self.tTemp = shallowcopy(self.tTargets)
...
function modifier_ember_spirit_1:OnIntervalThink()
	if IsServer() then
		local hAbility = self:GetAbility()
		while #self.tTemp > 0 do
			local hUnit = self.tTemp[1]
			if not IsValid(hUnit) or not hUnit:IsAlive() then
				table.remove(self.tTemp, 1)
			else
		...
		if #self.tTemp <= 0 then
			for i = #self.tTargets, 1, -1 do
				if not IsValid(self.tTargets[i]) or not self.tTargets[i]:IsAlive() then
...
function modifier_ember_spirit_1:ECheckState()
	return {

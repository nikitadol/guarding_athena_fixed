---@class ember_spirit_4 : eom_ability
ember_spirit_4 = eom_ability({})
...
function ember_spirit_4:OnScepterLevelup(iLevel)
	if iLevel == 1 then
		self:SetMaxCharges(self:GetSpecialValueFor("scepter_max_charges"), self:GetCooldown(self:GetLevel() - 1))
	end
end
function ember_spirit_4:OnScepterRemove()
	self:SetMaxCharges(self:GetSpecialValueFor("max_charges"), self:GetCooldown(self:GetLevel() - 1))
end
...
function ember_spirit_4_1:OnProjectileThink(vLocation)
	...
	for n, hTarget in pairs(tTargets) do
		if TableFindKey(self.tTargets, hTarget) == nil then
			...
			hCaster:FindAbilityByName("ember_spirit_0"):ApplyEmber(hTarget)
...
function ember_spirit_4_1:OnProjectileHit(hTarget, vLocation)
	...
	for n, hTarget in pairs(tTargets) do
		if TableFindKey(self.tTargets, hTarget) == nil then
			hCaster:DealDamage(hTarget, self, fDamage)
...
function modifier_ember_spirit_4_thinker:OnIntervalThink()
	...
	if IsValid(hAbility) then
		...
		for _, hUnit in ipairs(tTargets) do
			hCaster:DealDamage(hUnit, hAbility, damage, DAMAGE_TYPE_MAGICAL)
			hCaster:FindAbilityByName("ember_spirit_0"):ApplyEmber(hUnit)
		end

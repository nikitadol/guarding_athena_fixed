---@class ember_spirit_0 : eom_ability
ember_spirit_0 = eom_ability({})
...
function ember_spirit_0:ApplyEmber(hTarget, iStackCount)
	local hCaster = self:GetCaster()
	iStackCount = default(iStackCount, 1) + (hCaster:HasModifier("modifier_ember_spirit_3") and hCaster:FindAbilityByName("ember_spirit_3"):GetSpecialValueFor("bonus_stack") or 0)
	local duration = self:GetSpecialValueFor("duration")
	hTarget:AddNewModifier(hCaster, self, "modifier_ember_spirit_0_debuff", { duration = duration, iStackCount = iStackCount })
end
function ember_spirit_0:PlaySound(vPosition)
	local hCaster = self:GetCaster()
...
function modifier_ember_spirit_0:OnAttackLanded(params)
	if IsServer() then
		self:GetAbility():ApplyEmber(params.target)
	end
end
---------------------------------------------------------------------
...
function modifier_ember_spirit_0_debuff:OnDestroy()
	if IsServer() then
		...
		hAbility:PlaySound(hParent:GetAbsOrigin())
	end
end
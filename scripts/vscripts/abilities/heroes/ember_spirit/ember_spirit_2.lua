---@class ember_spirit_2 : eom_ability
ember_spirit_2 = eom_ability({})
function ember_spirit_2:OnSpellStart()
	...
	hCaster:Dash(vDirection, 350, 0, 0.3, nil, function()
...
function ember_spirit_2:OnProjectileHit_ExtraData(hTarget, vLocation, ExtraData)
	local hCaster = self:GetCaster()
	if hTarget then
		local flDamage = self:GetSpecialValueFor("damage") * (ExtraData.flChannelTime / self:GetChannelTime())
...
function modifier_ember_spirit_2:ECheckState()
	return {

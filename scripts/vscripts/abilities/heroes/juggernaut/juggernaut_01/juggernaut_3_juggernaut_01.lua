---@class juggernaut_3_juggernaut_01 : eom_ability
juggernaut_3_juggernaut_01 = eom_ability({})
...
function juggernaut_3_juggernaut_01:OnProjectileHit(hTarget, vLocation)
	...
	if hTarget then
		if hCaster:GetScepterLevel() >= 3 then
			hCaster:DealDamage(hTarget, self, damage, DAMAGE_TYPE_PHYSICAL, DOTA_DAMAGE_FLAG_IGNORES_PHYSICAL_ARMOR)
		else
			hCaster:DealDamage(hTarget, self, damage)
		end
	end
end
function juggernaut_3_juggernaut_01:OnScepterLevelup(iLevel)
	---@type CDOTA_BaseNPC
	local hCaster = self:GetCaster()
	if iLevel >= 3 then
...
function juggernaut_3_juggernaut_01:OnScepterRemove()
	---@type CDOTA_BaseNPC
	local hCaster = self:GetCaster()
...
----------------------------------------Modifier----------------------------------------
---@class modifier_juggernaut_3_juggernaut_01 : eom_modifier

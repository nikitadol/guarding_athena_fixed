...
function public:OnCreated(params)
	...
	if IsServer() then
		...
		self.bUseMana = params.bUseMana == 1 and true or false
		...
		if self.iOrderType == DOTA_UNIT_ORDER_CAST_POSITION or self.iOrderType == DOTA_UNIT_ORDER_CAST_TARGET then
			...
			ExecuteOrder(hParent, DOTA_UNIT_ORDER_MOVE_TO_POSITION, hParent:GetAbsOrigin() + vDirection)
		end
	end
end
...
function public:OnIntervalThink()
	...
	if self.flTime >= self.flCastPoint and self.bSucceeded == false then
		self.bSucceeded = true
		self:OnSuccess()
	end
	if self:GetRemainingTime() <= 0 and self.bSucceeded == true then
		self:Destroy()
	end
	if self.bSucceeded == false then
		if
...
function public:OnSuccess()
	...
	if IsServer() then
		if self.bSucceeded then
			hAbility:UseResources(self.bUseMana, false, self.bUseCooldown)
			-- 回调
			if self.iOrderType == DOTA_UNIT_ORDER_CAST_TARGET then
				hParent:SetCursorCastTarget(self.hTarget)
			elseif self.iOrderType == DOTA_UNIT_ORDER_CAST_POSITION then
...
function public:OnOrder(params)
	...
	if params.unit == self:GetParent() and self:GetElapsedTime() > FrameTime() then
		if params.ability ~= nil then
			...
			local iBehavior = params.ability:GetBehaviorInt()

---@class modifier_pet_base : eom_modifier
modifier_pet_base = eom_modifier({
	Name = "modifier_pet_base",
	IsHidden = true,
	IsDebuff = false,
	IsPurgable = false,
...
function modifier_pet_base:OnDestroy()
	if IsServer() then
		if IsValid(self:GetParent()) and self:GetParent().GetPet then
...
function modifier_pet_base:ECheckState()
	return {

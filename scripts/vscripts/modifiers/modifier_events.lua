...
function public:OnAbilityExecuted(params)
	...
	if IsValid(params.unit) and params.unit.tSourceModifierEvents and params.unit.tSourceModifierEvents[MODIFIER_EVENT_ON_VALID_ABILITY_EXECUTED] then
		local tModifiers = params.unit.tSourceModifierEvents[MODIFIER_EVENT_ON_VALID_ABILITY_EXECUTED]
		for i = #tModifiers, 1, -1 do
			local hModifier = tModifiers[i]
			if IsValid(params.unit) and IsValid(hModifier) and hModifier.OnValidAbilityExecuted then
				if IsValid(params.ability) and not params.ability:IsItem() and not params.ability:IsToggle() and params.ability:ProcsMagicStick() then
					hModifier:OnValidAbilityExecuted(params)
				end
			else
				table.remove(tModifiers, i)
			end
	...
	if IsValid(params.target) and params.target.tTargetModifierEvents and params.target.tTargetModifierEvents[MODIFIER_EVENT_ON_VALID_ABILITY_EXECUTED] then
		local tModifiers = params.target.tTargetModifierEvents[MODIFIER_EVENT_ON_VALID_ABILITY_EXECUTED]
		for i = #tModifiers, 1, -1 do
			local hModifier = tModifiers[i]
			if IsValid(params.target) and IsValid(hModifier) and hModifier.OnValidAbilityExecuted then
				if IsValid(params.ability) and not params.ability:IsItem() and not params.ability:IsToggle() and params.ability:ProcsMagicStick() then
					hModifier:OnValidAbilityExecuted(params)
				end
			else
	...
	if tModifierEvents and tModifierEvents[MODIFIER_EVENT_ON_VALID_ABILITY_EXECUTED] then
		local tModifiers = tModifierEvents[MODIFIER_EVENT_ON_VALID_ABILITY_EXECUTED]
		for i = #tModifiers, 1, -1 do
			local hModifier = tModifiers[i]
			if IsValid(hModifier) and hModifier.OnValidAbilityExecuted then
				if IsValid(params.ability) and not params.ability:IsItem() and not params.ability:IsToggle() and params.ability:ProcsMagicStick() then
					hModifier:OnValidAbilityExecuted(params)
				end
			else
...
function public:OnDeath(params)
	...
	-- 触发击杀怪物事件（排除召唤物）
	if not params.unit:IsSummoned() and params.unit:GetClassname() == "npc_dota_creature" then
		if IsValid(params.attacker) and params.attacker.tSourceModifierEvents and params.attacker.tSourceModifierEvents[MODIFIER_EVENT_ON_ENEMY_KILLED] then
			local tModifiers = params.attacker.tSourceModifierEvents[MODIFIER_EVENT_ON_ENEMY_KILLED]
			for i = #tModifiers, 1, -1 do
				local hModifier = tModifiers[i]
				if IsValid(params.attacker) and IsValid(hModifier) and hModifier.OnEnemyKilled then
					hModifier:OnEnemyKilled(params)
				else
					table.remove(tModifiers, i)
				end
			end
		end
		if IsValid(params.unit) and params.unit.tTargetModifierEvents and params.unit.tTargetModifierEvents[MODIFIER_EVENT_ON_ENEMY_KILLED] then
			local tModifiers = params.unit.tTargetModifierEvents[MODIFIER_EVENT_ON_ENEMY_KILLED]
			for i = #tModifiers, 1, -1 do
				local hModifier = tModifiers[i]
				if IsValid(params.unit) and IsValid(hModifier) and hModifier.OnEnemyKilled then
					hModifier:OnEnemyKilled(params)
				else
					table.remove(tModifiers, i)
				end
		...
		if tModifierEvents and tModifierEvents[MODIFIER_EVENT_ON_ENEMY_KILLED] then
			local tModifiers = tModifierEvents[MODIFIER_EVENT_ON_ENEMY_KILLED]
			for i = #tModifiers, 1, -1 do
				local hModifier = tModifiers[i]
				if IsValid(hModifier) and hModifier.OnEnemyKilled then
					hModifier:OnEnemyKilled(params)
				else
					table.remove(tModifiers, i)
				end

...
function public:game_over(iPlayerID, bWin)
	local iLoseShard = GameRules:GetCustomGameDifficulty() < 4 and 0 or Rounds:GetRoundNumber() * (GameRules:GetCustomGameDifficulty() - 2)
	...
	if data and data.status == 0 then
		if GameRules:State_Get() >= DOTA_GAMERULES_STATE_GAME_IN_PROGRESS then
			...
			if router == "skin.equip" then
				...
				-- 	hHero.gift = true	--待完善
				-- 	-- 新方法
				-- 	hHero:GameTimer(0.1, function()
				-- 		-- AssetModifiers:ReplaceWearables(tItemData.ItemName, heroEntity)
				-- 		hHero:AddNewModifier(hHero, nil, "modifier_" .. tSkinData.ItemName, nil)
				-- 	end)
				-- end
			end
			if router == "courier.equip" then
				local hHero = PlayerResource:GetSelectedHeroEntity(iPlayerID)
				if hHero.pet then
					hHero.pet:RemoveSelf()
				end
				local tCourierData = GetEquippedCourier(iPlayerID)
				hHero.pet = NewPet(tCourierData.ItemName, hHero, tonumber(tCourierData.Experience))
			end
		end
		return {
			status = data.status
...
function GetEquippedCourier(iPlayerID)
	local tData = CustomNetTables:GetTableValue("service", "player_courier_" .. iPlayerID) or {}
...
function EachEquippedParticles(iPlayerID, callback)
	local tData = CustomNetTables:GetTableValue("service", "player_particle_" .. iPlayerID) or {}
	for i, tItemData in pairs(tData) do
		if tItemData.Equip == "1" then
			callback(tItemData)
		end
...
function GetEquippedSkin(iPlayerID, sHeroName)
	local tData = CustomNetTables:GetTableValue("service", "player_skin_" .. iPlayerID) or {}
	for i, tItemData in pairs(tData) do
...
function EachPlayerGameplay(iPlayerID, callback)
	local tData = CustomNetTables:GetTableValue("service", "player_gameplay_" .. iPlayerID) or {}
	for i, tItemData in pairs(tData) do
		callback(tItemData)
	end
end
...
function EachPlayerOther(iPlayerID, callback)
	local tData = CustomNetTables:GetTableValue("service", "player_other_" .. iPlayerID) or {}
	for i, tItemData in pairs(tData) do
		callback(tItemData)
	end
end